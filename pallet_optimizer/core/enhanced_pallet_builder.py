...
class EnhancedPalletBuilder:
    ...
    def _convert_to_config(
        ...
        # CRITICAL: Dimensions can NEVER be less than the base pallet size
        # Even if boxes only use 31" width, the pallet itself is still 40" wide
        if placed_boxes:
            ...
            used_length = round(max_x, 1)
            used_width = round(max_z, 1)
            used_height = round(max_y, 1)
            
            # CRITICAL: Enforce minimum pallet base dimensions
            # The pallet physically takes up space even if boxes don't use it all
            min_length = pallet_dims[0]  # 48"
            min_width = pallet_dims[1]   # 40"
            
            actual_dims = (
                max(used_length, min_length),  # Length: at least 48"
                max(used_width, min_width),    # Width: at least 40"
                used_height                     # Height: as calculated
            )
        else:
